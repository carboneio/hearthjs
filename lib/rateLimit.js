...
const PROFILE_OPTIONS = ['max', 'window', 'key', 'scope', 'dryRun', 'onLimit', 'message', 'maxKeys', 'headers']
...
const rateLimit = {
  ...
  _normalizeProfile: function (name, options) {
    ...
    if (options.headers !== undefined && typeof options.headers !== 'boolean') {
      throw new Error(`Rate limit profile '${name}': headers must be a boolean`)
    }
    ...
    return Object.freeze({
      ...
      // Emit the RateLimit-* headers on this route (off by default, like the
      // global net's APP_RATE_LIMIT_GLOBAL_HEADERS)
  ...
  _routeMiddleware: function (spec, routeId) {
    ...
    return this._buildMiddleware(_profile, _limiter, { runtimeToggle: true, headers: _profile.headers })
  },
