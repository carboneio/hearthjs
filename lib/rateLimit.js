...
const rateLimit = {
  ...
  /**
   * Override what the APP_RATE_LIMIT* env keys cannot express on the global
  ...
  _routeMiddleware: function (spec, routeId) {
    ...
    if (_profile.scope === 'shared') {
      if (_profile.shared.limiter === null) {
        _profile.shared.limiter = createLimiter(_profile.max, _profile.window, _profile.maxKeys)
      }

      _limiter = _profile.shared.limiter
    } else {
      _limiter = createLimiter(_profile.max, _profile.window, _profile.maxKeys)
    }

    return this._buildMiddleware(_profile, _limiter, {})
  },

  /**
   * The global limiter, from the APP_RATE_LIMIT* keys. Env vars win over the
   * config file, like the other APP_* keys.
   * @param {Object} config Loaded server config, may be null
   * @return {Function|null} Middleware, or null when the feature is off
   */
  _globalMiddleware: function (config) {
    ...
    const _enabled = _read('APP_RATE_LIMIT')

    if (_isTrue(_enabled) === false) {
      // A security switch must never be silently ignored: '1', 'yes' or 'TRUE'
      // is someone who believes the limiter is on
      if (_enabled !== undefined && _enabled !== false && _enabled !== 'false') {
        logger.log(`APP_RATE_LIMIT='${_enabled}' is not recognized, the rate limiter is OFF (only 'true' turns it on)`, 'warn')
      }
    ...
    const _profile = this._normalizeProfile('global', {
      max: this._parsePositiveNumber('APP_RATE_LIMIT_MAX', _read('APP_RATE_LIMIT_MAX'), DEFAULT_MAX),
      window: this._parsePositiveNumber('APP_RATE_LIMIT_WINDOW', _read('APP_RATE_LIMIT_WINDOW'), DEFAULT_WINDOW),
      maxKeys: this._parsePositiveNumber('APP_RATE_LIMIT_MAX_KEYS', _read('APP_RATE_LIMIT_MAX_KEYS'), DEFAULT_MAX_KEYS),
      key: this._globalOptions.key,
      onLimit: this._globalOptions.onLimit
    })

    const _skipRaw = _read('APP_RATE_LIMIT_SKIP')
    ...
    return this._buildMiddleware(_profile, createLimiter(_profile.max, _profile.window, _profile.maxKeys), {
      headers: _isTrue(_read('APP_RATE_LIMIT_HEADERS')),
