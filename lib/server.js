...
const server = {
   ...
   */
  _reset: function () {
    // Profiles and roles are declared again by the project beforeInit on the
    // next start
    rateLimit._reset()
    roles._reset()
    this._app = null
  ...
  _init: function (env, callback) {
    ...
    this._callFunctionIfExists('beforeInit', [], (err) => {
      ...
      database.init({
        ...
        connectionTimeout: this.config.APP_DATABASE_CONNECTION_TIMEOUT,
        idleTimeout: this.config.APP_DATABASE_IDLE_TIMEOUT,
        applicationName: this.config.APP_DATABASE_APPLICATION_NAME
      }, (err) => {
          if (err) {
            return callback(err)
          }
              ...
              this._callFunctionIfExists('init', [this._app], (err) => {
                ...
                this._initAddons(this._addons, 0, (err) => {
                  ...
                  let interval = setInterval(() => {
                    if (this._apiReady === false) {
                      return
                    }

                    // Stopped the moment readiness is seen, before any of the
                    // async work below: a tick used to fire again while the crons
                    // were still loading and load every one of them a second
                    // time, and a cron that failed returned without stopping the
                    // interval at all, leaving it firing for the life of the
                    // process while run() had already reported the error
                    clearInterval(interval)

                    // Every route is registered: the policies can be checked as
                    // a whole. Refused here, the server never reaches listen().
                    // Required inline, like the other api uses in this file: api
                    // requires server back, and a top level require is a cycle.
                    const _api = require('./api')
                    let _policyError = null

                    // A throw here would escape run() entirely, leaving no callback
                    try {
                      _policyError = roles._checkRoutes(_api.routes())
                    } catch (e) {
                      _policyError = e
                    }

                    if (_policyError !== null) {
                      // A failed boot must leave nothing behind: the addons are
                      // the sharp one, since useAddon() exits the process on a
                      // duplicate, so the retry died instead of showing the
                      // report the operator needs. _reset also clears the roles
                      // table and the rate limit profiles beforeInit declares.
                      this._reset()
                      _api._reset()
                      return callback(_policyError)
                    }
    ...
    fs.access(configPath, fs.constants.F_OK, (err) => {
      ...
      fs.readFile(configPath, 'utf-8', (err, data) => {
        ...
        try {
          ...
          // The config can change where the logs go, and the logger caches that
          logger._resetLogOutputSetting()
          // Opens the log file, or drops the one already opened, to match
  ...
  _initMiddleware: function () {
    ...
    // Before cookie parsing and long before body parsing: a rejected request
    // must cost as close to zero as possible. Null when APP_RATE_LIMIT_GLOBAL is off,
    // so a disabled limiter costs nothing at all.
    const _rateLimitMiddleware = rateLimit._globalMiddleware(this.config)

    if (_rateLimitMiddleware !== null) {
      this._app.use(_rateLimitMiddleware)
    }
  ...
  /**
   * A string passed to next() is a refusal written for the caller, anything else
   * is a server fault. That tells apart the status, the log level and what the
   * client is told, so it has one owner.
   * @param {String|Error} err Value passed back by a handler or a middleware
   * @returns {Boolean} True when the error is a server fault
   */
  ...
  /**
   * Status an error answers with: err.code is the hearthjs convention, err.status
   * and err.statusCode the express one. A status node would refuse is ignored.
   * @param {String|Error} err Value passed back by a handler or a middleware
   * @returns {Number} HTTP status to answer with
   */
  _statusForError: function (err) {
    if (err === null || err === undefined) {
      return 500
    }

    // Any status node accepts, since an application picks this one itself
    if (Number.isInteger(err.code) === true && err.code >= 100 && err.code <= 999) {
      return err.code
    }

    // express only trusts these two inside the error range
    if (Number.isInteger(err.status) === true && err.status >= 400 && err.status < 600) {
      return err.status
    }

    if (Number.isInteger(err.statusCode) === true && err.statusCode >= 400 && err.statusCode < 600) {
      return err.statusCode
    }

    // A fault answers 500, like express: a 4xx tells the caller it made a
    // mistake, and hides a server failure from everything watching 5xx
    return this._isFault(err) ? 500 : 400
  },
  ...
  _handleError: function (err, req, res) {
    const _code = this._statusForError(err)

    // A fault's text carries table names, file paths and stack details, a
    // refusal is written for the caller
    const _isFault = this._isFault(err)
    const _expose = _isFault === false || this._env !== 'prod' || (err !== null && err !== undefined && err.expose === true)
  ...
  _startServer: function (env, options, callback) {
    this._loadConfig(env, options, (err) => {
      ...
      this._init(env, (err) => {
        ...
        const _onListenError = (err) => {
          _listenFailed = true
          return callback(err)
        }

        this._httpServer.once('error', _onListenError)
        ...
        this._server = this._httpServer.listen(this.config.APP_SERVER_PORT, () => {
          ...
          // Startup is over: a later error must not call run() again
          this._httpServer.removeListener('error', _onListenError)
          this._httpServer.on('error', (err) => {
            logger.log(`Server error: ${err.toString()}`, 'error', { logDate: false })
          })
  ...
  _logStartup: function (env) {
    ...
    const _output = logger._getLogOutput()
    // Naming a file nobody writes to would be a lie
    let _logs = logger._mustLogInFile() ? `${logger._getLogFilePath()} · output ${_output}` : `output ${_output}`

    if (logger._mustLogOnStdout() === true && logger._isUnderJournald() === true) {
      // Each line reaches the journal carrying its level as a real priority
      _logs += ' · journald priorities'
    }
  ...
  _logStartupWarnings: function (env) {
    ...
    // Nothing reaches the log file or stdout: worth saying out loud once, and
    // it has to go out directly since logger.log() now writes nowhere
    if (logger._getLogOutput() === 'none') {
      process.stderr.write('hearthjs: APP_LOG_OUTPUT=none, logging is disabled\n')
    }

    if (logger._isStdoutSettingDeprecated() === true) {
      logger.log('APP_LOG_STDOUT is deprecated and ignored: APP_LOG_OUTPUT is set and wins', 'warn', _options)
    }

    // Without this the process writes nothing to journald, so a crash leaves a
    // stack trace with no request context around it
    if (env === 'prod' && logger._getLogOutput() === 'file') {
      // Under systemd the journal would also carry the level of every line
      const _gain = (logger._isUnderJournald() === true) ? ', with each line at its own priority' : ''

      logger.log(`logs are written to the file only: journalctl will show nothing. Set APP_LOG_OUTPUT=both${_gain}`, 'warn', _options)
    }
