...
const server = {
  ...
  _logStartup: function (env) {
    ...
    _row('logs', logger._mustLogInFile() ? `${logger._getLogFilePath()} · output ${logger._getLogOutput()}` : `output ${logger._getLogOutput()}`)
  ...
  _logStartupWarnings: function (env) {
    ...
    if (env === 'prod' && logger._getLogOutput() === 'file') {
      logger.log('logs are written to the file only: journalctl will show nothing. Set APP_LOG_OUTPUT=both', 'warn', _options)
    }
