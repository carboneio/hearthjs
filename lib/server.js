...
const server = {
  ...
  _logStartup: function (env) {
    ...
    const _shutdown = this._isGracefulShutdownEnabled() ? this._humanMs(this._getShutdownTimeout()) : 'none'
    ...
    _row('database', `${_read('APP_DATABASE_NAME')}@${_read('APP_DATABASE_HOST')}:${_read('APP_DATABASE_PORT')} · statement timeout ${this._humanMs(_dbTimeout)}`)
    _row('loaded', `${_nbApi} apis · ${_api._nbRouteDeclared} routes · ${_nbCron} crons · ${this._addons.length} addons`)
    _row('timeouts', `request ${this._humanMs(this._getRequestTimeout())} · shutdown ${_shutdown}`)
