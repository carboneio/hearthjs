const server = require('./server')
const database = require('./database')
const validation = require('./validation')
const addon = require('./addon')
const logger = require('./logger')
const rateLimit = require('./rateLimit')
const roles = require('./roles')

// Everything hearthjs itself reads in a schema. Any other key must name a
// registered addon, or it is a typo.
const SCHEMA_OPTIONS = new Set([
  'after', 'before', 'function', 'in', 'middleware', 'out', 'permission', 'query', 'rateLimit', 'successMsg'
])

const api = {
  _apiList: Object.create(null),
  _currentApiName: '',
  _nbRouteDeclared: 0,
  _nbRouteServed: 0,

  /**
   * Reset all API variables
   */
  _reset: function () {
    this._apiList = Object.create(null)
    this._currentApiName = ''
  },

  /**
   * Define all API routes defined by the user
   * @param {String} apiName Name of the API
   * @param {Function} definition Define all API routes
   */
  define: function (apiName, schemas, definition) {
    this._currentApiName = apiName

    if (this._apiList[apiName] !== undefined) {
      logger.log(`${apiName} has already been declared`, 'error', { logDate: false })
      this._checkApiIsReady()
      return
    }

    // Register API
    this._apiList[apiName] = {
      schemas: schemas,
      routes: {}
    }
    definition(this)
  },

  /**
   * Add a GET route to the server
   * @param {String} route Route like express
   * @param {Object} schemaName Route schema
   */
  get: function (route, schemaName) {
    this._addRoute('GET', route, schemaName)
  },

  /**
   * Add a POST route to the server
   * @param {String} route Route like express
   * @param {Object} schemaName Route schema
   */
  post: function (route, schemaName) {
    this._addRoute('POST', route, schemaName)
  },

  /**
   * Add a PUT route to the server
   * @param {String} route Route like express
   * @param {Object} schemaName Route schema
   */
  put: function (route, schemaName) {
    this._addRoute('PUT', route, schemaName)
  },

  /**
   * Add a DELETE route to the server
   * @param {String} route Route like express
   * @param {Object} schemaName Route schema
   */
  delete: function (route, schemaName) {
    this._addRoute('DELETE', route, schemaName)
  },

  /**
   * Add a route to the server
   * @param {String} method Route method
   * @param {String} route Route
   * @param {Object} schemaName Details of route
   */
  _addRoute: function (method, route, schemaName) {
    if (schemaName === undefined) {
      logger.log(`You must specify a function or a schema for ${route}`, 'error', { logDate: false })
      this._checkApiIsReady()
      return
    }

    // Check route begin with '/'
    if (route.startsWith('/') === false) {
      route = `/${route}`
    }

    if (this._apiList[this._currentApiName].routes[`${method} ${route}`] !== undefined) {
      logger.log(`${method} ${route} has already been defined`, 'error', { logDate: false })
      this._checkApiIsReady()
      return
    }

    // Looked up before the route is counted: a route counted but never served
    // leaves _nbRouteDeclared above _nbRouteServed, and startup waits forever
    let realSchema = this._apiList[this._currentApiName].schemas[schemaName]

    if (realSchema === undefined) {
      logger.log(`Schema ${schemaName} of API ${this._currentApiName} was not found`, 'error', { logDate: false })
      this._checkApiIsReady()
      return
    }

    // Resolved before the route is counted, like the schema: a broken rate limit
    // or an unresolvable policy drops the route (404) at startup rather than
    // shipping it unlimited, or unguarded
    let _rateLimitMiddleware = null
    let _roleMiddleware = null

    try {
      // Resolved before the rate limit, which registers a live limiter as a side
      // effect: a policy that throws after it would leave that limiter behind for
      // a route nobody ever serves
      if (realSchema.permission !== undefined) {
        _roleMiddleware = roles._routeMiddleware(realSchema.permission)
      }

      if (realSchema.rateLimit !== undefined) {
        _rateLimitMiddleware = rateLimit._routeMiddleware(realSchema.rateLimit, `${method} ${route}`)
      }
    } catch (e) {
      logger.log(`${method} ${route}: ${e.message}`, 'error', { logDate: false })
      this._checkApiIsReady()
      return
    }

    // Resolved before the route is counted, like the schema and the rate limit.
    // Addons are the only authentication hearthjs offers and they are opted into
    // by name, so `needAuthentcation` would otherwise ship the route unguarded,
    // indistinguishable from one that is public on purpose.
    const _unknownKeys = Object.keys(realSchema).filter((key) => {
      return SCHEMA_OPTIONS.has(key) === false && server._addons.findIndex(elem => elem.schemaKeyName === key) === -1
    })

    if (_unknownKeys.length > 0) {
      logger.log(`${method} ${route}: unknown schema key ${_unknownKeys.join(', ')}`, 'error', { logDate: false })
      this._checkApiIsReady()
      return
    }

    this._nbRouteDeclared += 1

    // Register route with schema
    this._apiList[this._currentApiName].routes[`${method} ${route}`] = schemaName

    let middlewareList = realSchema.middleware || []

    // The addons this schema uses, in the order the project registered them, NOT
    // the order the schema happens to list its keys in: writing `getIdCustomer`
    // above `needAuthentication` used to run them that way round, which is an
    // accident of the object literal deciding a security order. Registration
    // order is written once, in the project's index.js, and applies everywhere.
    const _addonKeys = server._addons
      .filter((elem) => realSchema[elem.schemaKeyName] !== undefined)
      .map((elem) => elem.schemaKeyName)

    this._initAllAddons(_addonKeys, 0, route, realSchema, (err) => {
      if (err) {
        logger.log(err.toString(), 'error', { logDate: false })
        return
      }

      // A closure, not `new Function(...)`: concatenating the names into a
      // source string let a name containing a quote inject code.
      const _apiName = this._currentApiName
      const _schemaName = schemaName
      const func = (req, res, next) => {
        req.hearthjs = { apiName: _apiName, schemaName: _schemaName }
        next()
      }

      const _handlers = [route, func]

      // The limiter runs first: authentication addons, DB lookups and user
      // middleware all sit behind the gate
      if (_rateLimitMiddleware !== null) {
        _handlers.push(_rateLimitMiddleware)
      }

      // Call addons before user defined middleware
      if (_addonKeys.length > 0) {
        _handlers.push((req, res, next) => {
          let schema = this._apiList[req.hearthjs.apiName].schemas[req.hearthjs.schemaName]
          this._execAddons(_addonKeys, 0, req, res, schema, (err, code) => {
            if (err) {
              code = code || 400
              return res.status(code).json(this.createResponse(req, false, {}, err.toString()))
            }

            return next()
          })
        })
      }

      // After the addon runner, never among the addons: their order is the schema's
      // key order, so an authentication addon could otherwise run second and an
      // anonymous caller would get 403 where they are owed 401. A project that
      // authenticates in `middleware` instead gets the guard first and refuses
      // everyone — loudly wrong, rather than quietly open.
      if (_roleMiddleware !== null) {
        _handlers.push(_roleMiddleware)
      }

      _handlers.push(middlewareList)

      if (realSchema.function === undefined) {
        _handlers.push((req, res) => {
          let apiName = req.hearthjs.apiName
          let schemaName = req.hearthjs.schemaName
          this._execRoute(req, res, this._apiList[apiName].schemas[schemaName])
        })
      } else {
        _handlers.push(realSchema.function)
      }

      server._app[method.toLowerCase()].apply(server._app, _handlers)

      this._nbRouteServed += 1
      this._checkApiIsReady()
    })
  },

  /**
   * Init every addon the schema uses, in order
   * @param {Array} keys Addon schema keys, in registration order
   * @param {Integer} index keys index
   * @param {String} route Route associated with the schema
   * @param {Object} schema Schema with keys and values
   * @param {Function} callback
   */
  _initAllAddons: function (keys, index, route, schema, callback) {
    if (index >= keys.length) {
      return callback(null)
    }

    const _key = keys[index]
    const _addon = server._addons[server._addons.findIndex(elem => elem.schemaKeyName === _key)]

    addon.initSchemaAddon(_addon, schema[_key], database, route, schema, (err) => {
      if (err) {
        return callback(new Error(`Error while initializing schema for addon ${_key}: ${err.toString()}`))
      }

      // Advance, rather than call back: this used to stop after the first key,
      // so the second addon of a schema was never initialized, and a schema
      // whose first key was not an addon initialized none of them
      return this._initAllAddons(keys, index + 1, route, schema, callback)
    })
  },

  /**
   * Check if all routes declared has been served and update server
   */
  _checkApiIsReady: function () {
    if (this._nbRouteDeclared === this._nbRouteServed) {
      server._setApiReady(true)
    }
  },

  /**
   * Execute the schema of the route
   * @param {Object} req Req from request
   * @param {Object} res Res from request
   * @param {Object} schema Details route
   */
  _execRoute: function (req, res, schema) {
    // TODO: remove this if
    if (typeof schema === 'function') {
      schema(req, res)
    } else {
      // If schema in is defined, check all fields are valid
      if (schema.in !== undefined) {
        let result = validation.checkObject(schema.in, req.body)

        // Return error if there is
        if (result.valid === false) {
          return res.status(400).json(this.createResponse(req, false, {}, result.message))
        }

        this._execBefore(req, res, schema)
      } else {
        this._execBefore(req, res, schema)
      }
    }
  },

  /**
   * A string passed back by a handler is a refusal written for the caller,
   * anything else is a fault. Same rule as server._handleError.
   * @param {String|Error} err Value the handler passed back
   * @returns {Boolean} True when it is a server fault
   */
  _isFault: function (err) {
    return server._isFault(err)
  },

  /**
   * A refusal is the caller's mistake, not a server fault: logging it at error
   * buries the real failures
   * @param {String|Error} err Value the handler passed back
   * @returns {String} Log level to report it at
   */
  _levelForHandlerError: function (err) {
    return this._isFault(err) ? 'error' : 'warn'
  },

  /**
   * Message a refused request can carry back. A fault holds table names and file
   * paths, so production gets a generic one, exactly like _handleError.
   * @param {String|Error} err Value the handler passed back
   * @returns {String} Message for the caller
   */
  _messageForClient: function (err) {
    if (err === null || err === undefined) {
      return 'An error occured'
    }

    if (this._isFault(err) === false || server._env !== 'prod' || err.expose === true) {
      return err.toString()
    }

    return 'An error occured'
  },

  /**
   * Check all keys in schema and exec addons
   * @param {Array} keyArray Array of schema keys
   * @param {Integer} index keyArray index
   * @param {Object} req Req from request
   * @param {Object} res Res from request
   * @param {Object} schema Schema used
   * @param {Function} callback
   */
  _execAddons: function (keyArray, index, req, res, schema, callback) {
    if (index >= keyArray.length) {
      return callback(null)
    }

    let key = keyArray[index]
    let addonIndex = server._addons.findIndex(elem => elem.schemaKeyName === key)

    if (addonIndex !== -1) {
      // Exec addonc exec function
      addon.execAddon(server._addons[addonIndex], schema[key], database, req.originalUrl, schema, req, res, (err, code) => {
        if (err) {
          logger.log(err.toString(), this._levelForHandlerError(err))
          return callback(err, code)
        }

        return this._execAddons(keyArray, index + 1, req, res, schema, callback)
      })
    } else {
      return this._execAddons(keyArray, index + 1, req, res, schema, callback)
    }
  },

  /**
   * Run the schema before handler, then its query, then the after handler
   * @param {Object} req Req from request
   * @param {Object} res Res from request
   * @param {Object} schema Route schema
   */
  _execBefore: function (req, res, schema) {
    // Execute before
    this._mayExec(req, res, schema.before, 'before', (err, code) => {
      if (err) {
        logger.log(err.toString(), this._levelForHandlerError(err))
        return res.status(code || 400).json(this.createResponse(req, false, {}, this._messageForClient(err)))
      }

      if (schema.query !== undefined) {
        // Execute user query
        let model = (schema.out === undefined) ? null : schema.out

        database.exec(schema.query, req, model, (err, result, formatted) => {
          if (err) {
            logger.log(err.toString(), 'error')
            return res.status(500).json(this.createResponse(req, false, {}, 'An error occured'))
          }

          // Execute after
          this._execAfter(req, res, formatted, schema)
        })
      } else {
        // No query wanted, execute after
        this._execAfter(req, res, {}, schema)
      }
    })
  },

  /**
   * Execute route after function
   * @param {Object} req req from Request
   * @param {Object} res res from Request
   * @param {Object} data Data return by SQL query
   * @param {Object} schema Route schema
   */
  _execAfter: function (req, res, data, schema) {
    this._mayExec(req, res, schema.after, 'after', data, (err, code, newData) => {
      if (err) {
        logger.log(err.toString(), this._levelForHandlerError(err))
        return res.status(code || 400).json(this.createResponse(req, false, {}, this._messageForClient(err)))
      }
      data = (newData !== undefined) ? newData : data
      return res.status(code || 200).json(this.createResponse(req, true, data, (schema.successMsg === undefined) ? '' : schema.successMsg))
    })
  },

  /**
   * Check the wanted function exists and execute it
   * @param {Object} req req from request
   * @param {Object} res res from request
   * @param {Function} func Function to execute if exists
   * @param {String} type before/after
   * @param {Object} data Object to pass if exists
   * @param {Function} callback
   */
  _mayExec: function (req, res, func, type, data, callback) {
    // Init var if there is no data
    if (typeof data === 'function') {
      callback = data
      data = {}
    }

    if (func === undefined) {
      return callback()
    }

    // The handler may reject after having already called next(), so the
    // callback is guarded against being run twice
    let _alreadyCalled = false
    const _callbackOnce = function (err, code, newData) {
      if (_alreadyCalled === true) {
        return
      }

      _alreadyCalled = true
      return callback(err, code, newData)
    }

    let _result = null

    // Call function
    try {
      if (type === 'before') {
        _result = func(req, res, _callbackOnce)
      } else {
        _result = func(req, res, data, _callbackOnce)
      }
    } catch (e) {
      return _callbackOnce(e)
    }

    // A rejected async handler used to become an unhandled rejection, which
    // terminates the process on Node >= 15.
    if (_result !== null && _result !== undefined && typeof _result.then === 'function') {
      _result.catch((e) => _callbackOnce(e))
    }
  },

  /**
   * Every route declared, with the schema it was declared with. The one place
   * route -> schema is resolved: a caller listing routes, checking their policy
   * or asserting which ones are public reads the same resolution the router used.
   * @return {Array} [{ method, path, api, schemaName, schema }, ...]
   */
  routes: function () {
    const _routes = []

    for (const _apiName of Object.keys(this._apiList)) {
      const _api = this._apiList[_apiName]

      for (const _key of Object.keys(_api.routes)) {
        const _space = _key.indexOf(' ')

        _routes.push({
          method: _key.slice(0, _space),
          path: _key.slice(_space + 1),
          api: _apiName,
          schemaName: _api.routes[_key],
          schema: _api.schemas[_api.routes[_key]]
        })
      }
    }

    return _routes
  },

  /**
   * Create a response object
   * @param {Boolean} success Request result
   * @param {Object} data Data to return
   * @param {String} message Message to send
   */
  createResponse: function (req, success, data, message) {
    return {
      success: success,
      data: data,
      message: message
    }
  }

}

module.exports = api
