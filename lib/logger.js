...
// Query parameters whose VALUE is a secret: OAuth codes, reset/invite tokens,
// API keys, signatures. Their value is replaced before a URL reaches the logs,
// so a `?token=...`/`?code=...` never lands in the log file in the clear.
const SENSITIVE_QUERY_PARAMS = new Set([
  'token', 'access_token', 'refresh_token', 'id_token', 'code', 'secret',
  'client_secret', 'api_key', 'apikey', 'key', 'password', 'pwd', 'pass',
  'sig', 'signature', 'state', 'session', 'sessionid', 'auth', 'authorization'
])
...
const logger = {
  ...
  /**
   * Redact the value of any sensitive query parameter before a URL is logged.
   * The common case (no query string) returns the url untouched at no cost.
   * The path and the non-sensitive parameters are kept, so the line stays
   * useful for debugging without leaking a credential.
   * @param {String} url The raw request URL
   * @return {String} The URL with sensitive values replaced by [redacted]
   */
  _redactUrl: function (url) {
    ...
    const _path = url.slice(0, _index)
    const _query = url.slice(_index + 1)

    const _redacted = _query.split('&').map((pair) => {
      const _eq = pair.indexOf('=')

      if (_eq === -1) {
        return pair
      }

      const _name = pair.slice(0, _eq).toLowerCase()

      return SENSITIVE_QUERY_PARAMS.has(_name) ? `${pair.slice(0, _eq)}=[redacted]` : pair
    }).join('&')

    return `${_path}?${_redacted}`
  },
  ...
  _logMiddleware: function () {
    ...
    // Resolved once, not per request: reading the config on every line would
    // undo the logger's own hot-path work
    const _redactUrl = this._mustRedactQuery()
    ...
    return (req, res, next) => {
      ...
      if (_logStart) {
        this.log(`--> ${req.method} ${_redactUrl(req.url)} [${req.hearth_uid}]`, 'info')
      }
      ...
      res.on('finish', () => {
        ...
        let _message = `${req.method} ${_redactUrl(req.url)} ${_status} ${this._formatDuration(_durationMs)}`
  ...
  /**
   * Should sensitive query-parameter values be redacted from logged URLs?
   * Off by default: whole query strings are logged, which is easier to debug.
   * Turn it on with APP_LOG_REDACT_QUERY=true (config file or environment) so a
   * secret passed as ?token=/?code= never lands in the log in the clear.
   */
  _mustRedactQuery: function () {
    if (process.env.APP_LOG_REDACT_QUERY !== undefined) {
      return process.env.APP_LOG_REDACT_QUERY === 'true'
    }

    // Required here and not at the top of the file: server.js requires logger.js
    const _server = require('./server')

    return _server.config !== null && _server.config.APP_LOG_REDACT_QUERY === true
  },
