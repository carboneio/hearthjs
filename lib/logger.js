...
const logger = {
  ...
  _mustLogOnStdout: function () {
    if (this._stdoutSetting === null) {
      this._stdoutSetting = this._resolveStdoutSetting()
    }

    return this._stdoutSetting
  },

  /**
   * Read the setting from the environment, then from the project config
  ...
  /**
   * Forget the cached stdout setting. Called when the logger is initialised and
   * once the project config is loaded, since that can turn it on.
   */
  _resetStdoutSetting: function () {
    this._stdoutSetting = null
  },
