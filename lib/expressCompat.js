...
const REQUEST_DESCRIPTORS = {
  ...
  query: {
    configurable: true,
    // Parsed on first read only: most requests never look at the query string
    get: function () {
  ...
  ip: {
    configurable: true,
    get: function () {
      // An explicit assignment (some proxy middlewares set req.ip) wins
      if (this._hearthIp !== undefined) {
        return this._hearthIp
      }

      if (trustProxy === true) {
        const _forwarded = this.headers['x-forwarded-for']

        if (_forwarded !== undefined) {
          // The RIGHTMOST entry: the hop our own proxy appended. The leftmost
          // (express' choice) is client-supplied when the proxy appends rather
          // than overwrites, letting an attacker pick their own identity.
          const _index = _forwarded.lastIndexOf(',')
          const _value = (_index === -1) ? _forwarded.trim() : _forwarded.slice(_index + 1).trim()
      ...
      return (this.socket !== undefined) ? this.socket.remoteAddress : undefined
    },
    // A setter, so `req.ip = x` never throws (a getter-only prop would, in
    // strict mode) — keeps the request API assignment-safe like express'
    set: function (value) {
      this._hearthIp = value
    }
  },
  hostname: {
    configurable: true,
...
function decorateRequest (req) {
  ...
  // restana already exposes `path`, `params` and `originalUrl`
...
function resSetHeader (name, value) {
  // the other callers pass a constant, this one builds a message: only pay for
  // it once it is needed, send() sets six to eight headers per file served
  if (this.headersSent === true) {
    alreadyAnswered(this, `set the header ${name}`)
    return this
  }
  ...
  try {
    ...
    // throw ERR_INVALID_CHAR. Caller driven, so warn: it must not flood the log
    require('./logger').log(`Dropped the header ${name}: ${e.message}`, 'warn')
    return this
  }
}
...
function resSet (field, value) {
  ...
  if (field.toLowerCase() === 'content-type') {
    ...
    _value = mime.contentType(_value) || _value
  }
...
function resType (value) {
  return this.set('content-type', mime.contentType(value) || 'application/octet-stream')
}
...
function resCookie (name, value, options = {}) {
  ...
  const _serialized = cookie.stringifySetCookie({ ..._options, name, value: _value })
...
function decodeParams (req) {
  ...
  for (const _key in _params) {
    // for..in walks the prototype chain: a polluted Object.prototype would
    // otherwise be decoded and copied onto the params as an own property
    if (Object.hasOwn(_params, _key) === false) {
      continue
    }
