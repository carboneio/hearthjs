...
/**
 * How long a change can go unnoticed. Read on every call so setting
 * HEARTH_WATCH_INTERVAL after require still applies.
 */
function getPollInterval () {
  const _fromEnv = parseInt(process.env.HEARTH_WATCH_INTERVAL, 10)

  return (Number.isInteger(_fromEnv) && _fromEnv > 0) ? _fromEnv : 500
}
        ...
        if (_isDirectory) {
          ...
          for (let j = 0; j < watchedDirectory.length; j++) {
            if (watchedDirectory[j].test(path.join(directoryPath, files[i]))) {
              ...
              fs.watchFile(path.join(_completePath, files[i]), { persistent: true, interval: getPollInterval() }, (oldStat, newStat) => {
                // mtimeMs avoids building a Date on every poll, and rounding keeps
                // the millisecond granularity so sub millisecond jitter is ignored
                if (Math.round(newStat.mtimeMs) !== Math.round(oldStat.mtimeMs)) {
