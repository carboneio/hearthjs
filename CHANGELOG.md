...
- Per-route limits with the new `rateLimit` schema key: an inline `{ max, window, key, scope, dryRun, onLimit, message, maxKeys, headers }` object, or the name of a profile declared with `hearthjs.rateLimit.define(name, options)` in `beforeInit`. `scope: 'shared'` (default) makes routes on the same profile share one budget per key; the limiter runs before addons and user middleware. A rejected request gets `429` + `Retry-After` with the standard response body. `headers: true` emits the `RateLimit-*` headers on that route (off by default, like the global net).
...
- **A dropped connection is retried once, safely.** The pool already reconnects for later queries; now a query that hits a connection-level error (socket reset, `08xxx`, `57P0x` admin/crash shutdown) is retried once on a fresh connection. A failed `connect()` means the query never ran, so any query is retried; a connection lost mid-query is only retried for a plain read (`SELECT`, not a locking read, `SELECT INTO` or a `WITH` CTE), so a write is never replayed. Real query errors (`statement_timeout`, deadlock, constraint violations) are never retried, and a broken client is destroyed instead of returned to the pool.
- **TCP keepalive on the pool.** `keepAlive` is on (probes start after 10s idle), so a connection a NAT, firewall or load balancer silently dropped is detected and evicted instead of failing a request — a client-side socket setting, independent of the server (PostgreSQL, TimescaleDB, PgBouncer).
- **The pool identifies itself** in `pg_stat_activity` via `application_name` (default `hearthjs`), and its size and timeouts are configurable: `APP_DATABASE_POOL_MAX` (default 10), `APP_DATABASE_IDLE_TIMEOUT` (10000 ms), `APP_DATABASE_CONNECTION_TIMEOUT` (10000 ms), `APP_DATABASE_APPLICATION_NAME`. The defaults match pg's own, so existing deployments are unchanged.
...
- **736 tests** (was 431), green on Node 26. Full run **38s -> 20s**.
...
- **`hearthjs.httpClient` can stop following redirects.** `new hearthjs.httpClient(email, pwd, { followRedirect: false })` (or `client.followRedirect = false`) surfaces the `3xx` status and the `Location` header instead of following, so redirect assertions (login gating, OAuth authorize/consent, open-redirect safety) no longer need a hand-rolled `fetch(..., { redirect: 'manual' })` — and the client keeps forwarding its cookie jar.
- **`hearthjs.datasets` reset helpers.** `datasets.setDefaultSeed([...])` sets a base seed; `datasets.resetHook(extra)` returns a `beforeEach` function that cleans then inserts the seed plus the extra sets — the project wires it in, hearthjs never touches the test runner's globals. `datasets.clean()` now retries a Postgres deadlock (`40P01`) internally, up to twice, so a fire-and-forget write from the previous test that straddles the next `TRUNCATE` no longer needs an app-side wrapper.
- **`hearthjs.test` race-safe waiting.** `hearthjs.test.waitUntil(fn, cb, opts)` and `hearthjs.test.waitForRow(sql, params, isReady, cb, opts)` poll a condition (or a query) instead of sleeping a fixed delay — for the fire-and-forget writes that settle after the API has already answered.
